...
#             res = aux.getResults(db, startDate, endDate)
#             for item in res:
#                 temp = dict()
#                 temp["id"] = str(item.id).encode('utf8')
#                 temp["location"] = str(item.locationId).encode('utf8')
#                 temp["rain"] = str(item.rain).encode('utf8')
#                 temp["date"] = str(item.date.strftime("%m/%d/%Y")).encode('utf8')
#                 temp["time"] = str(item.time.strftime("%H:%M:%S")).encode('utf8')
#                 yield json.dumps(temp) + ','
#     return Response(generate(), mimetype='text')

# @app.route('/large2', methods=['GET', 'POST'])
# def sendResponsee():
#     print request
#     ipdb.set_trace()
#     startDate = datetime.date(2000,1,1)
